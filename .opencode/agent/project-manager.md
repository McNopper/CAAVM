...
3. **Plan** — `task_plan_sprint` to commit tickets into the sprint (`sprint-backlog`).
4. **Run** — workers `task_claim(role=...)`; you watch `task_board()` and
   `task_backlog()`; keep status current.
5. **Triage** — on `blocked` tickets (`task_set_blocked`), resolve internally (reassign,
...
(`task_set_blocked`). You triage: resolve internally, or — only if it crosses the human's
